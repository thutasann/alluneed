...
class CollectionController extends Controller
{
    ...
    public function subcategoryview($group_url, $cate_url, $subcate_url)
    {
        ...
        if (Request::get('sort') == 'price_asc') {
            ...
            $products = Products::whereIn('sub_category_id', $subcateid)->where('status', '0')->get();
        } else {
    ...
    {
        if (Auth::user()) {
            $products = Products::where('status', '!=', '2')->where('status', '0')->get();
            return view('frontend.collections.vendor-profile')->with('products', $products);
        } else {
            return redirect()->route('login')->with('pls-login', 'Please Login First');
        }
    }
